...
import { Badge, Button, Card, ErrorState, LoadingState, PageHeader, PageShell, type BadgeTone } from "@yuni/ui";
...
export function InteractCall({ avatarId }: { avatarId: string }) {
  ...
  if (avatarState.status === "loading") {
    return (
      <PageShell maxWidth="1120px">
        <LoadingState title="Cargando llamada" description="Estamos preparando el avatar." />
      </PageShell>
    );
  }
  ...
  if (avatarState.status === "error") {
    return (
      <PageShell maxWidth="760px">
        <ErrorState title="No pudimos cargar la llamada" description={avatarState.error} />
      </PageShell>
    );
  }
